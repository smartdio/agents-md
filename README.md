...
Optional cost-aware variant using GPT-6 sub-agents (verify current availability and rates first):

```text
Use https://github.com/smartdio/agents-md/blob/main/README.md as a reference to design a multi-agent collaboration workflow for the current project and document it in the project's AGENTS.md. Preserve the main agent's configured model and reasoning depth. Delegate bounded, independent work when its expected progress, context, or verification benefit justifies extra agent usage. For demanding coding, analysis, or review, consider an available `gpt-6-sol` sub-agent; for clear, repeatable tasks, consider `gpt-6-luna`; use direct tools for mechanical steps. Choose reasoning depth separately for each model and task, starting with `medium` for GPT-6 Sol or `high` for GPT-6 Luna when appropriate, and adjust based on task evidence. Compare current rates and expected validation or rework cost rather than assuming an older model is cheaper. Use only model and reasoning combinations supported by the current environment, preserve existing project constraints and the same acceptance standards, and reserve stronger models for work that needs them. Modify only the project's AGENTS.md; do not modify tools or global configuration.
```

This example follows current [OpenAI model guidance](https://learn.chatgpt.com/docs/models) for Sol and Luna; [sub-agent guidance](https://learn.chatgpt.com/docs/agent-configuration/subagents) notes their extra token use. Model selection and reasoning depth remain separate decisions. Check current [credit rates](https://learn.chatgpt.com/docs/pricing#token-rates) and your environment's available models before reusing named examples; a lower token rate does not prove lower total task cost.
